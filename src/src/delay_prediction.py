...
# de-dup multi-item orders to one row (take first payment record, sum item value)
df = df.drop_duplicates(subset='order_id')
print(f"Loaded {len(df):,} delivered orders with complete data")

# ---- Target: was it delivered late? ----
...
# keep category cardinality manageable - top 15 categories, rest 'other'
...
# ---- Seller historical late-rate feature (train-only, no leakage) ----
...
# shrink toward global mean for sellers with few orders (avoid overfitting on n=1 sellers)
...
# ---- Business framing: pick threshold that catches most late orders ----
# Default 0.5 threshold is wrong here -- late is the class we care about catching.
# We use a lower threshold to prioritize RECALL on "late" (cost of missing a late
# order = bad review + possible churn; cost of a false alarm = Ops double-checks
# a shipment that was fine, which is cheap).
from sklearn.metrics import precision_recall_curve
precisions, recalls, thresholds = precision_recall_curve(y_test, y_proba)

# pick threshold that gets recall >= 0.6 with best precision at that recall
target_recall = 0.60
...
print("\n" + "="*60)
print("MODEL PERFORMANCE -- default 0.5 threshold (misleading for this problem)")
print("="*60)
print(classification_report(y_test, y_pred_default, target_names=['On-time', 'Late']))

print("\n" + "="*60)
print(f"MODEL PERFORMANCE -- business threshold ({chosen_threshold:.3f}, tuned for recall)")
print("="*60)
print(classification_report(y_test, y_pred_business, target_names=['On-time', 'Late']))
print(f"ROC-AUC: {roc_auc_score(y_test, y_proba):.3f}")
print(f"Confusion matrix (business threshold):\n{confusion_matrix(y_test, y_pred_business)}")

tn, fp, fn, tp = confusion_matrix(y_test, y_pred_business).ravel()
print(f"\nBusiness read: of {tp+fn} truly late orders in the test set, we now catch {tp} "
      f"({tp/(tp+fn):.0%}) before they ship. "
      f"Cost: {fp} orders get flagged as 'at risk' that would have arrived fine "
      f"({fp/(fp+tn):.1%} false-alarm rate on on-time orders).")
...
print("\n" + "="*60)
print("TOP 15 FEATURE IMPORTANCES (what drives delay risk)")
print("="*60)
...
print("\nModel saved to outputs/delay_risk_model.pkl")
